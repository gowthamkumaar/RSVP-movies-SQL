...
FROM
	movie AS m
		INNER JOIN genre AS g
			ON m.id = g.movie_id 
-- Since it is asked to find the genre with highest number of movies in the last year (2019), a where clause is used.
WHERE 
	year = 2019
GROUP BY 
	genre
...
SELECT 
	title,
    avg_rating,
    ROW_NUMBER() OVER(ORDER BY avg_rating DESC) 'movie_rank'
FROM
	movie AS m
...
FROM
	movie AS m
		INNER JOIN ratings AS r
			ON m.id = r.movie_id
WHERE
	production_company IS NOT NULL AND avg_rating > 8	
GROUP BY
	production_company
ORDER BY
	movie_count DESC;
...
WHERE
    ...
    (year = 2017) AND (MONTH(date_published) = '03') AND
    (country LIKE '%USA%')
GROUP BY
	genre
ORDER BY
...
FROM
	movie AS m
        ...
        ON m.id = r.movie_id
WHERE
	(languages = 'German' AND country = 'Germany') OR 
    (languages = 'Italian' AND country = 'Italy')
    
    -- Here, the votes are counted based on German movies from Germany or Italian movies from Italy
        ...
        ON n.id = dm.name_id
            ...
            ON dm.movie_id = m.id
				...
				ON m.id = g.movie_id
                    ...
                    ON m.id = r.movie_id
WHERE
	genre IN (SELECT * FROM top_3_genres) AND avg_rating > 8
GROUP BY
	name
ORDER BY
	movie_count DESC
...
SELECT 
	...
	RANK() OVER(ORDER BY ROUND((SUM(avg_rating * total_votes)/SUM(total_votes)),2) DESC, total_votes DESC) 'actor_rank'
FROM
	names AS n
...
SELECT 
	...
	RANK() OVER(ORDER BY ROUND((SUM(avg_rating * total_votes) / SUM(total_votes)), 2) DESC, total_votes DESC) 'actress_rank'
FROM
	names AS n
		INNER JOIN role_mapping AS r
		ON n.id = r.name_id
            ...
            ON r.movie_id = rat.movie_id
                ...
                ON r.movie_id = m.id
WHERE
	country='India' AND category = 'actress' AND languages LIKE '%Hindi%'
GROUP BY
	name
HAVING
	COUNT(m.id) >= 3
...
SELECT
	genre,
    ROUND(AVG(duration),2) AS avg_duration,
    SUM(ROUND(AVG(duration),2)) OVER (ORDER BY AVG(duration)) AS 'running_total_duration',
				-- AVG(duration) is used to calculate the running total and moving average as mentioned
    AVG(ROUND(AVG(duration),2)) OVER (ORDER BY AVG(duration)) AS 'moving_avg_duration'
			-- Since ROWS argument is not passed, UNBOUNDED PRECEDING and CURRENT ROW becomes the default
FROM
	genre AS g
		INNER JOIN movie AS m
		ON g.movie_id = m.id
...
SELECT
    ...
    SUM(total_votes) AS total_votes,
    COUNT(r.movie_id) AS movie_count,
    ROUND(AVG(avg_rating),2) AS actress_avg_rating, -- as there are superhit movies with count > 1, average of rating is calculated
    RANK() OVER(ORDER BY COUNT(r.movie_id) DESC) AS 'actress_rank' -- actresses are ranked based on the count of superhit movies
		-- Tie breakers are not explicitly mentioned and hence they have not been used in the rank() function
FROM
	names AS n
		INNER JOIN role_mapping AS r
        ON n.id = r.name_id
